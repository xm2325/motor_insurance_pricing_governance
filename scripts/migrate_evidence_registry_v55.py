...
| v0.55 hosted numerical repeat | exact computational head `cc6babb…` ran on westus and eastus2; artifacts are not byte-identical, but maximum GLM relativity drift is **3.50×10^-9**, maximum log-gap drift **3.47×10^-9**, XGBoost fold relativities are exact and all preselected same-sign fractions are unchanged; no exact/bitwise reproducibility claim is made | `governance/rating_shape_repeat_evidence_v55.json`, `RESULTS_V55.md` |
...
- The two hosted repeats are not byte-identical; approximately 1e-9 GLM numerical drift is retained explicitly rather than rounded into an exact reproducibility claim. It does not change any v0.55 same-sign interpretation.
